...
public class PowerUp extends GameFigure {
    ...
    public PowerUp(float x, float y, int radius) { // don't need color anymore
        super(x, y);
    ...
    public void update() {
        ...
        if (super.x + radius > GameBoard.width) {
            if(pState instanceof PowerStateUsed){
                nextState();
            }
            dx = -dx;
            super.x = GameBoard.width - radius;
        ...
        if (super.y + radius > GameBoard.height) {
            if(pState instanceof PowerStateUsed){
                nextState();
            }
            dy = -dy;
            super.y = GameBoard.height - radius;
